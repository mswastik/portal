...
def genmodel(df5):
    ...
    for i in df5['code__id'].unique():
        k='forecast/'+str(int(i))+'.joblib'
        if path.exists(k):
            pass
        else:
            sl=df5[df5['code__id']==i].copy()
            trainx,testx,trainy,testy=simple_fe(sl)
...
def genforecast(df5):
    ...
    for i in df5['code__id'].unique():
        ...
        if path.exists(k):
            ...
            tt=reg.predict(12)
            for c in range(12):
                #df6=df6.append({'code_id':i,'fore_qty':reg[i].predict(k),'month':pd.to_datetime(datetime.date.today()- MonthBegin(n=0),
                #   'dem_month':pd.to_datetime(datetime.date.today()- MonthBegin(n=k)},ignore_index=True)
    ...
    #engine.execute(query, (df.id[i], df.name[i], df.age[i]))
